...
from signals import generate_golden_crossover_signals
...
def get_binance_data_by_requests(ticker, interval, start, end):
    ...
    while True:
        ...
        if data.empty:
            print(f"No data returned for {ticker} in the specified time range[start, end_u].")
            break
...
def generateSignalWithBinance(ticker, interval, start, end, all_signals):
    try:
        ...
        signals = generate_golden_crossover_signals(opens, closes, ma_values, rsi_values, ma_of_rsi_values)
